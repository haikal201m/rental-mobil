	...
	if (isset($_SESSION['sw'])) {
		...
		echo "<h1 class='page-header'>Welcome, $name</h1>";
			...
			if ($conn->query("SELECT*FROM data_absen WHERE id_user='$id_user'")->num_rows!==0) {
				$no=0;
			 	...
			 	while ($get_month=$query_month->fetch_assoc()) {
			      ...
			      if ($cek!==0) {
			        echo "<h3 class='sub-header'>Konfirmasi Saya - $month </h3>";
			           ...
			           <table class='table table-striped'>
			            <thead>
			               <tr>
			                <th>No</th>
			                <th>Hari, Tanggal</th>
			                <th>Waktu Pengambilan Mobil</th>
			                <th>Status</th>
			                <th>Waktu Pengembalian Mobil</th>
			                <th>Status</th>
			               </tr>
			            </thead>
			            <tbody>";
			          ...
			          while ($get_absen=$query_absen->fetch_assoc()) {
			            ...
			            $time_in = "$get_absen[jam_msk]";
			             if ($get_absen['jam_klr']==="") {
			             	$time_out = "<strong>Belum Konfirmasi</strong>";
			             } else {
			...
			$conn->close();
				...
				echo "<div class='alert alert-warning'><strong>Tidak ada Konfirmasi untuk ditampilkan.</strong></div>";
			}
	} else {
			...
			if ($conn->query("SELECT*FROM data_absen WHERE id_user='$id_user'")->num_rows!==0) {
				$no=0;
			 	...
			 	while ($get_month=$query_month->fetch_assoc()) {
			      ...
			      $cek = $query_absen->num_rows;
			      if ($cek!==0) {
			        echo "<h4 class='sub-header'><strong>Konfirmasi:</strong> $name<br><strong>Bulan:</strong> $month $year </h4>";
			        echo "<div class='table-responsive'>
			           <table class='table table-striped'>
			            <thead>
			               <tr>
