<h3 class='page-header'>Detail Konfirmasi Pelanggan</h3>
			...
			if ($query->num_rows!==0) {
				...
				while ($get_siswa = $query->fetch_assoc()) {
					...
					echo "<tr>
							...
							<td>$name</td>
							<td>$school</td>
							<td><a href='absensi&id_siswa=$id_siswa' title='Absensi $name'>Lihat Konfirmasii</a></td>
						</tr>";
				}
				echo "</tbody></table>";
				$conn->close();
			} else {
				echo "<div class='alert alert-danger'><strong>Tidak ada Pelanggan untuk ditampilkan</strong></div>";
			}
		}
	 ?>
</div>